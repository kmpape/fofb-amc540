...
int GSVD_BPM_to_float(const int * in_vec, gsvd_float * out_vec)
{
    int i;
    for (i = 0; i < GSVD_NY; i++) {
...
int GSVD_CM_to_int(const gsvd_float * in_vec, int * out_vec)
{
    int i;
    memset((int *)out_vec, 0, TOT_NUM_CM*sizeof(int));
    for (i = 0; i < GSVD_NS; i++) {
        int tmp = round(in_vec[i]*GSVD_SCALING_FACTOR_WRITE);
        out_vec[GSVD_SLOW_TO_BPM[i]] = T_sat(tmp, GSVD_LIMIT_WRITE);
    }
    for (i = 0; i < GSVD_NF; i++) {
        int tmp = round(in_vec[GSVD_NS_PAD+i]*GSVD_SCALING_FACTOR_WRITE);
