...
#if (MPC_SLOWFAST == 1)
#include "mpc/slowfast_data/OBS_data_c6678_NWORKERS6_x.h"
#else
#include "mpc/standard_data/OBS_data_c6678_NWORKERS6_x.h"
#endif /* MPC_SLOWFAST */
#else
#if (MPC_SLOWFAST == 1)
#include "mpc/slowfast_data/OBS_data_c6678_NWORKERS6_H2_x.h"
#else
#include "mpc/standard_data/OBS_data_c6678_NWORKERS6_H2_x.h"
#endif /* MPC_SLOWFAST */
#endif /* FGM_MPC_HORIZON */
#else
#if (FGM_MPC_HORIZON == 1)
#if (MPC_SLOWFAST == 1)
#include "mpc/slowfast_data/OBS_data_c6678_NWORKERS6_y.h"
#else
#include "mpc/standard_data/OBS_data_c6678_NWORKERS6_y.h"
#endif /* MPC_SLOWFAST */
#else
#if (MPC_SLOWFAST == 1)
#include "mpc/slowfast_data/OBS_data_c6678_NWORKERS6_H2_y.h"
#else
#include "mpc/standard_data/OBS_data_c6678_NWORKERS6_H2_y.h"
#endif /* MPC_SLOWFAST */
#endif /* FGM_MPC_HORIZON */
#endif /* XDIR */
...
void OBS_update_delta_xN_or_xd(const obs_float * restrict L_in,
                           const obs_float * restrict delta_y_in,
                           obs_float * restrict delta_x_out);
void OBS_update_xN_or_xd(const obs_float * restrict delta_x_in,
...
#pragma DATA_ALIGN(OBS_delta_xN_static, OBS_ARRAY_ALIGN)
...
#pragma DATA_ALIGN(OBS_x9_static,       OBS_ARRAY_ALIGN)
...
volatile obs_float OBS_delta_xN_static[OBS_DIM]={0.0};    // delta_x7 = Lx*delta_y
...
volatile obs_float OBS_x9_static[OBS_DIM]={0.0};      // 9 steps delay
...
volatile obs_float * volatile OBS_x9_local;
volatile obs_float * volatile OBS_delta_y_local;
volatile obs_float * volatile OBS_delta_xN_local;
...
volatile obs_float * volatile OBS_delta_xN_global;
...
volatile obs_float * volatile OBS_x9_global;
#pragma SET_DATA_SECTION()
...
#ifdef OBS_UNROLL
void OBS_update_delta_y(const obs_float * restrict Cx_in,
                        const obs_float * restrict xN_in,
                        const obs_float * restrict y_in,
                        const obs_float * restrict xd_in,
                        obs_float * restrict delta_y_out)
{
    ...
    _nassert((int) xN_in % OBS_ARRAY_ALIGN == 0);
...
#pragma UNROLL(1)
        for (j = 0; j < OBS_W_NCOLS; j += 4, ind_shift += 32)
        {
            ...
            __x128_t f4_in_vec = _f2to128(_amem8_f2_const(&xN_in[j]),
                                          _amem8_f2_const(&xN_in[j + 2]));
                        ...
                        obs_float * restrict delta_y_out)
{
    ...
    for (i = 0, ind_shift = 0; i < OBS_W_NROWS; i++, ind_shift+=OBS_W_NCOLS)
    {
        ...
        {
            tmp_row_res += Cx_in[ind_shift + j] * xN_in[j];
        }
...
// xi += Apowi * delta_xN
void OBS_update_xi(const obs_float * restrict delta_xN_in,
               const obs_float Apowi_in,
               obs_float * restrict xi_in_out)
{
    int i;
    _nassert((int) xi_in_out % OBS_ARRAY_ALIGN == 0);
    _nassert((int) delta_xN_in % OBS_ARRAY_ALIGN == 0);
    for (i = 0; i < OBS_W_NROWS; i++)
    {
        xi_in_out[i] += Apowi_in * delta_xN_in[i];
    }
}
...
void OBS_reset_master(void)
{
    ...
    OBS_vec_init((obs_float *)OBS_delta_xN_static,0.0);
...
#if (OBS_delay == 9)
    OBS_vec_init((obs_float *)OBS_x9_static,0.0);
#endif
    CACHE_wbL1d((void *) OBS_delta_y_static, OBS_BYTES_GLOBAL_ARRAYS, CACHE_WAIT);
    CACHE_wbL1d((void *) OBS_delta_xN_static, OBS_BYTES_GLOBAL_ARRAYS, CACHE_WAIT);
...
#if (OBS_delay == 9)
    CACHE_wbL1d((void *) OBS_x9_static, OBS_BYTES_GLOBAL_ARRAYS, CACHE_WAIT);
#endif
}
...
void OBS_initialize_master(void)
{
    ...
    OBS_delta_xN_global = &(OBS_delta_xN_static[0]);
...
#if (OBS_delay == 9)
    CACHE_invL1d((void *) &OBS_x9_static[0], OBS_BYTES_GLOBAL_ARRAYS, CACHE_WAIT);
#endif
}
...
#if (OBS_delay == 9)
    OBS_x9_local = &(OBS_x9_static[ind_shift]);
#endif
    OBS_delta_y_local = &(OBS_delta_y_static[ind_shift]);
    OBS_delta_xN_local = &(OBS_delta_xN_static[ind_shift]);
...
#if (OBS_delay == 9)
    CACHE_invL1d((void *) (&(OBS_x9_global[0])), OBS_BYTES_GLOBAL_ARRAYS, CACHE_WAIT);
#endif
...
void OBS_update_observer_worker(const obs_float * y_meas)
{
    ...
    /* Compute: delta_x8 = Lx*delta_y */
    OBS_update_delta_xN_or_xd((const obs_float *)(&(OBS_Lx_local[0])),
                              (const obs_float *)OBS_delta_y_global,
                              (obs_float *)OBS_delta_xN_local);

    /* Compute: delta_xd = Ld*delta_y */
    OBS_update_delta_xN_or_xd((const obs_float *)(&(OBS_Ld_local[0])),
                              (const obs_float *)OBS_delta_y_global,
                              (obs_float *)OBS_delta_xd_local);

#ifdef OBS_SYNC_EVERY_STEP
    CACHE_wbL1d((void *) (&(OBS_delta_xN_local[0])), OBS_BYTES_LOCAL_ARRAYS, CACHE_WAIT);
    CACHE_wbL1d((void *) (&(OBS_delta_xd_local[0])), OBS_BYTES_LOCAL_ARRAYS, CACHE_WAIT);
    CACHE_invL1d((void *) (&(OBS_delta_xN_global[0])), OBS_BYTES_GLOBAL_ARRAYS, CACHE_WAIT);
...
#endif
#if (OBS_delay == 9)
    OBS_update_xN_or_xd((const obs_float *)OBS_delta_xN_local, (obs_float *)OBS_x9_local);
    OBS_update_xN_or_xd((const obs_float *)OBS_delta_xd_local, (obs_float *)OBS_xd_local);
    OBS_update_xi((const obs_float *)OBS_delta_xN_local, OBS_Ax_pow_1, (obs_float *)OBS_x8_local);
    OBS_update_xi((const obs_float *)OBS_delta_xN_local, OBS_Ax_pow_2, (obs_float *)OBS_x7_local);
    OBS_update_xi((const obs_float *)OBS_delta_xN_local, OBS_Ax_pow_3, (obs_float *)OBS_x6_local);
    OBS_update_xi((const obs_float *)OBS_delta_xN_local, OBS_Ax_pow_4, (obs_float *)OBS_x5_local);
    OBS_update_xi((const obs_float *)OBS_delta_xN_local, OBS_Ax_pow_5, (obs_float *)OBS_x4_local);
    OBS_update_xi((const obs_float *)OBS_delta_xN_local, OBS_Ax_pow_6, (obs_float *)OBS_x3_local);
    OBS_update_xi((const obs_float *)OBS_delta_xN_local, OBS_Ax_pow_7, (obs_float *)OBS_x2_local);
    OBS_update_xi((const obs_float *)OBS_delta_xN_local, OBS_Ax_pow_8, (obs_float *)OBS_x1_local);
    OBS_update_xi((const obs_float *)OBS_delta_xN_local, OBS_Ax_pow_9, (obs_float *)OBS_x0_new_local);
#else
    ...
    OBS_update_xi((const obs_float *)OBS_delta_xN_local, OBS_Ax_pow_5, (obs_float *)OBS_x3_local);
    OBS_update_xi((const obs_float *)OBS_delta_xN_local, OBS_Ax_pow_6, (obs_float *)OBS_x2_local);
    OBS_update_xi((const obs_float *)OBS_delta_xN_local, OBS_Ax_pow_7, (obs_float *)OBS_x1_local);
    OBS_update_xi((const obs_float *)OBS_delta_xN_local, OBS_Ax_pow_8, (obs_float *)OBS_x0_new_local);
#endif
