...
/* Define master core number */
#define IPC_MASTER_CORENUM  (7)
...
void ipc_master_wait_for_slaves_after_init(void); // Can be called after ipc_master_init and ipc_slave_init, waits until the slaves set is_alive
void ipc_master_set_req(int req_val); // Set a request value for all slaves
void ipc_master_set_req_slave_i(int req_val, Uint32 slaveId); // Set a request value for slave slaveId
